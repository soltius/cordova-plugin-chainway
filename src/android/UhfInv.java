...
public class UhfInv extends CordovaPlugin {
  ...
  //     barcode2DWS = Barcode2DWithSoft.getInstance();
  //     if (null != barcode2DWS) {
  //       Toast.makeText(cordova.getActivity(), "barcode2DWS.isPowerOn(): " + barcode2DWS.isPowerOn(), Toast.LENGTH_LONG).show();
  //     } else {
  //       Toast.makeText(cordova.getActivity(), "barcode2DWS is null", Toast.LENGTH_LONG).show();
  //     }
  //   } catch (Exception e) {
  ...
  public boolean execute(String action, final JSONArray args, final CallbackContext callbackContext)
    ...
    try {
      ...
      if ("common".equals(android.os.Build.MODEL)) {
      ...
      else if ("C66".equals(android.os.Build.MODEL) || "P700".equals(android.os.Build.MODEL) || "C71".equals(android.os.Build.MODEL) || "P800".equals(android.os.Build.MODEL)
                     || "C72".equals(android.os.Build.MODEL) || "P810".equals(android.os.Build.MODEL) || "C76".equals(android.os.Build.MODEL) || "P820".equals(android.os.Build.MODEL)) {
        ...
        if (action.contains("Barcode2DWithSoft")) {
          ...
          if ("readEPCsFromBuffer".equals(action)) {
            ...
            // Toast.makeText(cordova.getActivity(), "Toast: C72 no action matched", Toast.LENGTH_SHORT).show();
            callbackContext.error("Chainway no action matched!");
            return false;
          }
        }
      } else {
        // Toast.makeText(cordova.getActivity(), "Toast: No device matched", Toast.LENGTH_SHORT).show();
  ...
  private void init(CallbackContext callbackContext) {
    try {
      ...
      if (this.mReader.init()) {
        callbackContext.success("?????????UHF???????????????");
      } else {
        // toastMessage("Failure: UHFC72.init() return false");
        callbackContext.error("?????????UHF???????????????");
      }
    } catch (Exception e) {
